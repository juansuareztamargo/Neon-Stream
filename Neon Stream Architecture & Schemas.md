...
Even though the final deliverable will be compiled into a single `index.html` file, the internal structure and logical separation of concerns follow this modular architecture.

- Implementation note: `style.css` and modules under `scripts/` are logical components during development and may be bundled or inlined into `index.html` for the final single-file deliverable.
...
- All input is matched case-insensitively.
- Spanish accent characters are matched exactly when present in the target token (for example, "código" must include "ó").
- Each `pool` item is one target token; tokens may contain spaces (for example, "ancho de banda").
- Dead-key behavior is only visual guidance for the teacher overlay; gameplay compares final produced characters.
...
- Levels 15-17: Numbers and accents.
- Levels 18-20: Cyberpunk boss levels (high speed).

Timing contract:

- `spawnRate` is milliseconds between spawn attempts.
- `fallSpeed` is a difficulty multiplier applied to the renderer base falling velocity.
- The `fallSpeed` dip at Level 9 is intentional to ease transition from characters to words.
